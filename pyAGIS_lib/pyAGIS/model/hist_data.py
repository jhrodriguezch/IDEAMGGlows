...
class Fews_data:
    def __init__(self, dict_var):
        ...
        self.time_series_obs           = self.dict_var['FEWS URL SERIES']

    # Properties
    @property
    def time_series_obs(self):
        return self._time_series_obs
    @time_series_obs.setter
    def time_series_obs(self, input):
        req = urllib.request.urlopen(input)
        if req.status >= 400:
            raise Exception('El servidor de FEWS presento error.')
        
        rv = req.read().decode('utf-8')
        files = np.unique(re.findall('[0-9]*[a-zA-Z]*\.json', rv))
        stations = [re.findall('\d*', ii)[0][2:] for ii in files]

        df = pd.DataFrame({'files' : files,
                           'stations' : stations})
        df['json directory'] = input + '/' + df['files']
        
        self._time_series_obs = df
    ...
    def get_fews_obs_from_station(self, station_code):

        # Extract data
        obs = self.get_fews_data_from_station(station_code, col='obs')
        sen= self.get_fews_data_from_station(station_code, col='sen')

        # Built result variable
        rv = obs.copy()
        rv.rename(columns={'data' : 'obs'}, inplace=True)
        rv['sen'] = sen['data']
        rv['data'] = rv[['obs', 'sen']].mean(skipna=True, axis=1)
        rv.drop(['obs', 'sen'], axis=1, inplace=True)

        # Fix daily
        rv['__day__'] = rv.index.day
        rv['__month__'] = rv.index.month
        rv['__year__'] = rv.index.year

        rv = rv.groupby(['__year__', '__month__', '__day__']).mean()
        rv.reset_index(inplace=True)
        rv['date'] = pd.to_datetime(rv['__year__'].astype(str) + '/' + rv['__month__'].astype(str) + '/' + rv['__day__'].astype(str), format='%Y/%m/%d')
        rv.set_index('date', inplace=True)
        rv.drop(['__year__', '__month__', '__day__'], axis=1, inplace=True)
        return rv


    def get_fews_data_from_station(self, station_code, col):
        # Search station in url data 
        station_dataframe = self.time_series_obs.loc[self.time_series_obs['stations'] == station_code]
        if len(station_dataframe) != 1:
            print('Error in staion code.')
            return pd.DataFrame({'files'          : [''],
                                 'stations'       : [''],
                                 'json directory' : ['']})

        raw_data_station = json.load(urllib.request.urlopen(station_dataframe['json directory'].values[0]))[col]
        data_station = pd.DataFrame(raw_data_station['data'])
        data_station.columns = ['date', 'data']
        
        # Fix column data
        data_station['data'].astype('float') 
        data_station['date'] = pd.to_datetime(data_station['date'], format='%Y/%m/%d %H:%M')
        data_station.dropna(how='all', inplace=True)
